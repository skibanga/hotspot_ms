...
def _resolve_voucher(voucher_code: str):
    voucher_name = frappe.db.get_value(
        "Hotspot Voucher", {"voucher_code": voucher_code}, "name"
    )
    if not voucher_name:
        return None
...
def _compute_opennds_return_token(hid: str, faskey: str) -> str:
    return hashlib.sha256(
        f"{(hid or '').strip()}{(faskey or '').strip()}".encode()
    ).hexdigest()
...
def _push_wireguard_authentication(
    ...
    if not nas_name or not mac or not session_minutes:
        return False
...
def _sync_voucher_status(voucher) -> bool:
    ...
    if (
        voucher.expires_on
        and get_datetime(voucher.expires_on) <= now
        and voucher.status != "Expired"
    ):
        voucher.status = "Expired"
        changed = True
    elif _is_data_exhausted(voucher) and voucher.status not in {
        "Used",
        "Expired",
        "Blocked",
    }:
...
def _validate_device_reuse(
    voucher, mac_address: str | None = None, ip_address: str | None = None
) -> dict[str, Any] | None:
    """
    ...
    if bound_mac:
        ...
        if not incoming_mac:
            return _error(
                "Client MAC is required for this voucher", "MISSING_CLIENT_MAC"
            )
        return None
    ...
    if (
        bound_ip
        and incoming_ip
        and incoming_ip != bound_ip
        and voucher.status in {"Active", "Used", "Expired"}
    ):
...
def _build_status_url(
    session_id: str, voucher_code: str, upstream_redir: str | None = None
) -> str:
...
def _append_query(base_url: str, query: dict[str, str]) -> str:
    ...
    existing_query.update(
        {k: v for k, v in query.items() if v is not None and str(v).strip() != ""}
    )
    return urlunsplit(
        (
            parsed.scheme,
...
@frappe.whitelist()
def generate_voucher_from_sms_payment(
    amount: int | str,
    phone_number: str,
    reference: str,
    ...
    """
    Called by the Auto Vocha Flutter app when a Mixx by Yas payment SMS is received.
    Generates a voucher for the matching plan and returns the voucher code.
    """
    amount = int(flt(amount))

    # --- Find plan by name or by price ---
    if plan_name:
        plan_exists = frappe.db.exists(
            "Hotspot Plan", {"name": plan_name, "enabled": 1}
        )
        if not plan_exists:
            # Try matching by plan_name field
            plan_name_match = frappe.db.get_value(
                "Hotspot Plan", {"plan_name": plan_name, "enabled": 1}, "name"
            )
            if not plan_name_match:
                return _error(f"No active plan found with name: {plan_name}")
            plan_name = plan_name_match
    else:
        # Fallback: match by price
        plan_name = frappe.db.get_value(
            "Hotspot Plan", {"price": amount, "enabled": 1, "is_free": 0}, "name"
        )
        if not plan_name:
            return _error(f"No active plan found for amount: {amount}")

    # --- Deduplicate: check if this reference was already processed ---
    existing = (
        frappe.db.get_value(
            "Hotspot Voucher",
            {"custom_sms_reference": reference},
            ["name", "voucher_code"],
            as_dict=True,
        )
        if frappe.db.has_column("Hotspot Voucher", "custom_sms_reference")
        else None
    )

    if existing:
        return _success(
            "Voucher already exists for this payment",
    ...
    try:
        if frappe.db.has_column("Hotspot Voucher", "custom_sms_reference"):
            frappe.db.set_value(
                "Hotspot Voucher", voucher.name, "custom_sms_reference", reference
            )
    except Exception:
        pass
...
def activate_paid_access(
    ...
    voucher_name = (tx.get("voucher") or "").strip()
    voucher = (
        frappe.get_doc("Hotspot Voucher", voucher_name)
        if voucher_name and frappe.db.exists("Hotspot Voucher", voucher_name)
        else None
    )
    if not voucher:
...
def verify_voucher(
    ...
    device_error = _validate_device_reuse(
        voucher, mac_address=mac_address, ip_address=ip_address
    )
    if device_error:
        return device_error
    ...
    if flt(voucher.data_limit_mb) > 0:
        remaining_mb = max(
            round(flt(voucher.data_limit_mb) - flt(voucher.data_used_mb), 2), 0
        )
...
def activate_voucher(
    ...
    if existing_open_session:
        ...
        if open_mac and mac_address and open_mac != mac_address:
            return _error(
                "Voucher is already active on another device", "DEVICE_MISMATCH"
            )
    ...
    session_mins = _remaining_session_minutes(voucher.expires_on) or cint(plan_doc.validity_value)
...
def build_opennds_redirect(
    ...
    if gatewayaddress and hid:
        ...
        redirect_url = _append_query(
            auth_base, {"tok": return_token, "redir": status_url}
        )
        return _success(
            "Redirect URL prepared",
...
@frappe.whitelist(allow_guest=True)
def session_status(
    voucher_code: str | None = None, session_id: str | None = None
) -> dict[str, Any]:
    ...
    if not flt(s.get("total_mb")):
        ...
        s["total_mb"] = total_mb
        frappe.db.set_value(
            "Hotspot Session", s["name"], "total_mb", total_mb, update_modified=False
        )
...
def logout_session(session_id: str) -> dict[str, Any]:
    session_name = frappe.db.get_value(
        "Hotspot Session", {"session_id": session_id}, "name"
    )
    if not session_name:
    ...
    if not flt(session.total_mb):
        session.total_mb = _to_mb(
            flt(session.input_octets) + flt(session.output_octets)
        )
    session.save(ignore_permissions=True)
...
@frappe.whitelist()
def request_session_deauth(
    session_id: str, reason: str | None = None
) -> dict[str, Any]:
    """
    ...
    session = frappe.get_doc("Hotspot Session", session_name)
    if session.terminate_cause and session.terminate_cause.startswith(
        DEAUTH_PENDING_PREFIX
    ):
...
def sync_session_usage(
    ...
    for rec in records:
        ...
        if input_octets < flt(
            session_doc.get("input_octets") or 0
        ) or output_octets < flt(session_doc.get("output_octets") or 0):
...
def get_random_active_ad(mac_address: str | None = None) -> dict[str, Any]:
    ...
    if mac_address:
        ...
        if free_plans:
            ...
            if voucher and (
                cint(voucher[0].ad_slices_used) >= max_slices
                or voucher[0].status == "Expired"
            ):
                return _error(
                    frappe._(
                        "You have reached your daily limit of free access today. Please buy a package to continue!"
                    ),
                    "FREE_LIMIT_REACHED",
                )
    ...
    best_candidates = [
        ad for ad in valid_ads if cint(ad["views_count"]) == lowest_views
    ]
...
def log_ad_view_and_claim_slice(
    ...
    if existing_voucher:
        ...
        if (
            cint(voucher_doc.ad_slices_used) >= max_slices
            or voucher_doc.status == "Expired"
        ):
            return _error(
                "You have reached your daily limit of free access.",
                "FREE_LIMIT_REACHED",
            )
    ...
    result["message"] = frappe._(
        "Ad watched successfully! Internet unlocked for next {0} {1}."
    ).format(slice_duration_val, frappe._(plan_doc.get("validity_unit") or "Minutes"))
    return result
...
def create_advertiser_ad(
    ...
    if exists:
        return _error(
            "An ad with this title already exists. Please choose a different title.",
            "DUPLICATE_TITLE",
        )
...
def upload_ad_media(file_name: str, file_data: str) -> dict[str, Any]:
    ...
    file_doc = save_file(
        fname=file_name, content=decoded_data, dt="Hotspot Ad", dn="Temp", is_private=0
    )
...
def login_advertiser(email: str, password: str) -> dict[str, Any]:
    ...
    if not frappe.db.exists("User", email):
        return {
            "ok": False,
            "message": frappe._("Email not found. Please register first."),
        }
    ...
    try:
        ...
        return {
            "ok": False,
            "message": frappe._("An error occurred during authentication."),
        }
